...
export function SiteNav({
  ...
  name?: string;
  email?: string | null;
  /** Dark/mysterious public homepage */
  veil?: boolean;
}) {
  ...
  return (
    ...
    >
      <div className="mx-auto flex h-14 max-w-[1180px] items-center justify-between gap-6 px-5 md:px-8">
        ...
        >
          <a href="#lijn" className={`nav-link ${veil ? "hover:text-[var(--veil-ink)]" : "hover:text-[var(--ink)]"}`}>
            De lijn
          </a>
          {email ? (
            <Link href="/radar" className={`nav-link ${veil ? "hover:text-[var(--veil-ink)]" : "hover:text-[var(--ink)]"}`}>
              Desk
            </Link>
          ) : null}
        </nav>
